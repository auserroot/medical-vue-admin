        ...
        <div class="bottom">
          <template>
            ...
            >
              ...
              <el-table-column label="用户名" width="190" prop="nametitle">
                <template slot-scope="scope">
                  <input
                    type="text"
                    class="inpchange"
                    v-model="tableData[scope.$index].nametitle"
                  />
                </template>
              </el-table-column>
              <el-table-column prop="money" width="190" label="账号">
                <template slot-scope="scope">
                  <input
                    type="text"
                    class="inpchange"
                    v-model="tableData[scope.$index].money"
                  />
                </template>
              </el-table-column>
              <el-table-column prop="des" width="160" label="角色">
                <template slot-scope="scope">
                  <input
                    type="text"
                    class="inpchange"
...
export default {
  ...
  methods: {
    ...
    changeRow(index, rows) {
      this.$alert(
        '<input type="text" v-model="rows[index].usertitle">',
        "HTML 片段",
        {
    ...
    searchName() {
      ...
      this.expands.map((item, index) => {
        if (
          item.nametitle.indexOf(this.inp) != -1 ||
          item.money.indexOf(this.inp) != -1 ||
  ...
  created() {
    axios
      .get("http://rap2api.taobao.org/app/mock/276507/usertitle", {
        key: "usertitle"
      })
      .then(res => {
